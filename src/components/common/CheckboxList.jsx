...
*/
const CheckboxList = ({
  chkboxItems: inputCheckboxItems,
  handleOnCheckboxClicked,
  disableComponent,
}) => {
  const [checkboxItems, setCheckboxItems] = useState(inputCheckboxItems);
  const handleCheckboxOnClick = (e) => {
    let checkboxClickedIndex = e.target.id.split("-")[2];
    setCheckboxItems((prevDtls) => {
      let tempItems = [...prevDtls];
      tempItems[checkboxClickedIndex].checked =
        !tempItems[checkboxClickedIndex].checked;
    ...
    handleOnCheckboxClicked(
      checkboxClickedIndex,
      checkboxItems[checkboxClickedIndex]
    );
  };
  return (
    <div className={`relative flex w-full flex-col rounded-xl ${disableComponent ? "bg-gray-100" : "bg-white"} bg-clip-border text-gray-700 shadow-md`}>
      <nav className="flex min-w-[200px] flex-row flex-wrap gap-1 p-2 font-sans text-base font-normal text-blue-gray-700">
        {checkboxItems.map(({ label, checked }, index) => {
          return (
            <div
              role="button"
              className="bg-gray-200 flex items-center w-auto p-0 leading-tight transition-all rounded-lg outline-none text-start hover:bg-blue-gray-50 hover:bg-opacity-80 hover:text-blue-gray-900 focus:bg-blue-gray-50 focus:bg-opacity-80 focus:text-blue-gray-900 active:bg-blue-gray-50 active:bg-opacity-80 active:text-blue-gray-900"
              key={index}
            >
              <label
